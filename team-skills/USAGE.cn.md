...
说明如何使用 `team-skills/` 这层源码工作流定义。

这份文档面向维护者、工具适配者，以及需要查看原始 workflow 定义的高级使用者，而不是面向直接安装 bundle 的最终用户。
...
1. 先看本目录下的 [README.cn.md](README.cn.md)，选择合适的 workflow 家族
2. 再打开目标 workflow 的 README，理解它的职责
3. 查看 `workflow.yaml`，确认依赖和工具元数据
4. 如果需要确认实际分发行为，再去对照 `dist/` 下对应 bundle
...
部分源码 workflow 会故意保持模块化，并依赖其他 workflow 或外部 skills。这样的设计更利于维护，但也正因为如此，不应把这里当作最终用户默认安装入口。
