...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --check-dependencies)
      ...
      ;;
    -h|--help)
      echo "Usage: sh ./scripts/install-cursor.sh [--bundle <name>] [--project-root <path>] [--dry-run] [--backup] [--force] [--check-dependencies]"
      exit 0
      ;;
    *)
...
BUNDLE_ROOT="$REPO_ROOT/dist/cursor/bundles/$BUNDLE_FOLDER"
