...
This is an explicit opt-in workflow. Do not use it by default. Only use it when the user explicitly asks for it, names `superpowers-openspec-superpowers-workflow` or `$superpowers-openspec-superpowers-workflow`, or a repository policy explicitly requires it.
